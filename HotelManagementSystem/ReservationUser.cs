...
{
    public partial class ReservationUser : Form
    {
        public ReservationUser(string username)
        {
        ...
        {
            con.Open();
            string query = "select * from reservation where client= '"+username+"' order by id";
        ...
        {
            con.Open();
            string fillQuary = "select * from users where username = '"+username+"'";
            NpgsqlDataAdapter da = new NpgsqlDataAdapter(fillQuary, con);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        private void ReservationUser_FormClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
    }
